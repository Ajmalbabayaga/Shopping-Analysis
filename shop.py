...
spark = SparkSession.builder.appName("baba").getOrCreate()
...
table2.show()
state_delivery_count = table2.groupBy("state").agg(count("delivery_date").alias("delivery_count"))
state_delivery_count.show()
#create a bar graph
grouped_data_pandas = state_delivery_count.toPandas()
...
# ###############################################################################################
#
# # #3)what is most common age group among customers
#
table3 = join_out.select("customer_id","age")
common_age = table3.groupBy("age").agg(count("customer_id").alias("total_customer")).orderBy(desc("total_customer"))
common_age.show()
common_age_pandas = common_age.toPandas()
bar_width = 0.5  # You can adjust this value based on your preference
# Create a bar diagram with specified bar width
plt.bar(common_age_pandas['age'], common_age_pandas['total_customer'], width=bar_width, color='blue')
plt.xlabel('Age')
...
plt.show()
# #######################################################################################
# # #4)Calculate the average time it takes for orders to be delivered after they were placed and Which orders had the shortest and longest delivery times?
#
...
df1 = table8.groupBy("size", "colour").agg(count("product_id"))
...
##################################################################
## 10)Analyze whether there are seasonal trends in product sales.
